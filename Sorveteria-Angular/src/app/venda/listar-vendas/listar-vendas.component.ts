...
export class ListarVendasComponent implements OnInit {
  ...
  modalDelete(vendas: Vendas) {
    const result$ = this.modal.showConfirm(
      'Confirma exclusão',
      'Desesa excluir permanentemente a venda?',
      'Confirmar',
      'Cancelar',
      'danger'
    );
    result$
      ...
      .pipe(
        take(1),
        switchMap((result) =>
          result ? this.vendaService.deleteVenda(vendas.id) : EMPTY
        )
      )
      .subscribe(
        (sucess) => {
          this.modal.handleMessage('Venda excluída com sucesso', 'success');
          this.findAllVenda();
        },
        (error) =>
          this.modal.handleMessage(
            'Erro ao excluir a venda, tente novamente mais tarde',
            'danger'
          )
      );
  }
